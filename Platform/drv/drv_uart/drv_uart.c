...
typedef struct {
    ...
    INT8U       rx_idle;       /* 空闲帧结束标志：环缓冲中有一帧完整数据待读 */
...
static void uart_rx_isr(INT8U com)
{
    ...
    /* IDLE：按手册读SR再读DR清除；置帧结束标志，上层凑齐一帧再整批读取 */
...
BOOLEAN DRV_UART_OpenUart(DRV_UART_CFG_T *cfg)
{
    ...
    PORT_ASSERT((cfg != 0), RETURN_FALSE);
    PORT_ASSERT((cfg->com < DRV_UART_COM_MAX), RETURN_FALSE);
    PORT_ASSERT((cfg->rx_len != 0 && cfg->rx_len <= DRV_UART_RX_BUF_MAX), RETURN_FALSE);
    PORT_ASSERT((cfg->tx_len != 0 && cfg->tx_len <= DRV_UART_TX_BUF_MAX), RETURN_FALSE);
...
{
    PORT_ASSERT((com < DRV_UART_COM_MAX), RETURN_FALSE);
...
BOOLEAN DRV_UART_WriteCharWait(INT8U com, INT8U data)
{
    ...
    PORT_ASSERT((com < DRV_UART_COM_MAX), RETURN_FALSE);
    PORT_ASSERT((huart != 0), RETURN_FALSE);
