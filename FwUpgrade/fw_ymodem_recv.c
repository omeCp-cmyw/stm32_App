...
********************************************************************************
*/
#define YM_MAX_REG            80           /* 注册表上限（对应野火MAX_REG） */
#define YM_SCAN_PERIOD        _TICK, 1     /* 接收扫描周期1ms（对应野火PERIOD_SCAN） */
...
/* 报文类型注册表（对应野火s_reg_tbl，freelist/usedlist单链管理） */
...
BOOLEAN FW_UPG_YM_Register(INT16U type, void (*handler)(INT16U type, INT8U *ptr, INT16U len))
{
    ...
    OS_ASSERT(handler != 0, RETURN_FALSE);                                     /* 处理函数不能为空 */
    OS_ASSERT(s_freelist != 0, RETURN_FALSE);                                  /* 注册表已满 */
...
**			: （对应野火HandleFrame校验通过后调HdlRecvData）。
** 参数		: 无
** 返回		: 无
********************************************************************/
static void ymodem_packet_done(void)
...
/*******************************************************************
** 函数名	: ymodem_scan_tmr
** 函数描述	: 接收扫描定时器回调（对应野火ScanTmrProc）：
...
void FW_UPG_YM_Init(void)
{
    ...
    /* 建立注册表（对应野火InitRecv的s_reg_tbl链化） */
    ...
    if (s_scantmr == 0xFF) {
        ...
        s_scantmr = OS_CreateTmr((void *)0, ymodem_scan_tmr);
    }
    OS_StartTmr(s_scantmr, YM_SCAN_PERIOD);
...
** 函数描述	: 接收一个字节并驱动组包状态机（对应野火RecvHandle），
**			: 报文头查注册表派发，报文完整后校验处理。
** 参数		: [in] byte: 接收字节
** 返回		: 无
********************************************************************/
static void ymodem_recv_byte(INT8U byte)
{
    ...
    switch (s_rcb.state) {
    case YM_STATE_HEAD:
        /* 遍历注册表查报文头启动组包，非协议字节忽略（对应野火HdlRecvData查usedlist） */
        curptr = s_usedlist;
        while (curptr != 0) {
            if (curptr->type == byte) {
