...
/*
********************************************************************************
* 固件升级主控：数据经 FW_UPG_WriteData 写入APP备份分区，
* 完成后 FW_UPG_Finish 校验并写升级标志，复位交Bootloader搬运。
********************************************************************************
*/
...
typedef struct {
    ...
    uint32_t  erased_addr;      /* 备份分区已擦除到的地址 */
...
** 函数描述	: 写入一段固件数据到备份分区，写入前按需擦除目标扇区，
**			: 内部处理4字节对齐缓冲，末包填充字节截断丢弃。
** 参数		: [in] data: 数据指针
**			: [in] len:  数据字节数（含填充）
** 返回		: 成功true，失败false
********************************************************************/
