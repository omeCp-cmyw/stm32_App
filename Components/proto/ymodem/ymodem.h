...
/*
********************************************************************************
* 串口Ymodem升级通道公共定义（原样移植自备份工程fw_ymodem.h，
* 接收组包与发送应答分文件实现：
* ymodem_recv.c / ymodem_send.c）：
* 设备为被升级方（接收方）：正常态不主动发串口，升级窗口发'C'握手，
* 超时自动退回静默正常态；
* 接收侧逐字节组包后查注册表派发，扫描由软件定时器驱动；
* 发送侧CELL_T节点+三链表内存池管理，
...
/* 报文头类型注册结构体（freelist/usedlist注册管理） */
