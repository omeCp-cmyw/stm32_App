...
/*
 * OneNET设备鉴权token生成（移植自wifi_pro的onenet_token.c）：
 * sign_str = "et\nmethod\nres\nversion"（无行尾换行），
...
/*******************************************************************
** 函数名	: hmac_md5
** 函数描述	: RFC2104 HMAC-MD5, 密钥超块长时先散列
** 参数		: [in] key: 密钥
**          : [in] key_len: 密钥长度
**          : [in] msg: 签名消息
**          : [out] digest: 16字节签名结果
...
{
    /* 大缓冲静态化：token生成是全局栈峰值最大路径(约1.1KB，
     * 逼近1KB栈极限且叠加中断必溢出)，静态化后函数不可重入，
     * 当前单线程调用点唯一，可安全使用 */
    static char res[96];
