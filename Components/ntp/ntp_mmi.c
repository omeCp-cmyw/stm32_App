...
/*******************************************************************************
** 函数名称    NtpPrintTime
** 函数说明    打印某一Unix时间戳的年月日时分秒
** 输入参数    tag: 时间制式名(UTC/BEIJING)
**             unix_ts: Unix时间戳
** 输出参数    无
** 返回参数    无
*******************************************************************************/
...
uint32_t NTP_MMI_NowUnix(void)
{
    ...
    return s_base_unix + (osal_get_time_ms() - s_base_ms) / 1000;
}
...
uint64_t NTP_MMI_NowMs(void)
{
    ...
    return (uint64_t)s_base_unix * 1000 + (osal_get_time_ms() - s_base_ms);
}
