...
** 文件名:     ota_proto.c
** 版权所有:   无
** 文件描述:   OneNET OTA协议层（HTTP请求拼装与应答解析）
**             移植自 STM32F4\stm32_App 工程
**
*********************************************************************************/
...
#define OTA_VERSION_BODY \
    "{\"s_version\":\"" APP_OTA_S_VERSION \
    "\", \"f_version\": \"" APP_OTA_F_VERSION "\"}"
                        ...
                        char *out, int out_size)
{
    ...
    used = snprintf(out, (size_t)out_size,
                    ...
                    "\r\n"
                    "%s",
                    product_id, device_name, APP_OTA_HOST,
                      ...
                      const char *version, char *out, int out_size)
{
    ...
    used = snprintf(out, (size_t)out_size,
                    ...
                    "Connection: keep-alive\r\n"
                    "\r\n",
                    product_id, device_name, version, APP_OTA_HOST, token);
    if (used < 0 || used >= out_size)
        return -1;
    return used;
}
                       ...
                       int tid, int step, char *out, int out_size)
{
    ...
    used = snprintf(out, (size_t)out_size,
                    ...
                    "%s",
                    product_id, device_name, tid,
                    APP_OTA_HOST, body_len, token, body);
    if (used < 0 || used >= out_size)
        return -1;
    return used;
}
...
int ota_check_parse(const char *resp, ota_task_t *task)
{
    ...
    DEBUG_INFO("[OTA] check result: code=%d tid=%d size=%d md5=%s",
